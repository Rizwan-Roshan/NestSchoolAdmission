...
Future<void> main() async
{
  ...
  runApp(MaterialApp(
    home: EnquiryViewonly(),
  ));
}

class EnquiryViewonly extends StatefulWidget {
  @override
...
class _EnquiryViewState extends State<EnquiryViewonly> {
  ...
  Future<void> _showEditDialog(data) async {
    ...
    enquirydateController.text =
        DateFormat('dd-MMM-yyy').format(data['dateOfEnquiry'].toDate()) ?? '';
            ...
            content: Column(
              ...
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(labelText: 'Mode of Enquiry'),
                ),
                TextField(
              ...
              TextButton(
                onPressed: () async {
                  ...
                  Navigator.push(
                    ...
                    return EnquiryViewonly();
                  },),);
                },
                /*final newName = parentNameController.text;
                  final newPhone = parentphoneNumberController.text;
                  final newGrade = gradeController.text;
                  final newComments = commentsController.text;
                  final newEmail = emailAddressController.text;

                  if (newName.isNotEmpty) {
                    // Update the data in Firestore
                    await updateDataInFirestore(data.id, {
                      'Parent\'s Name': newName,
                    ...
                    setState(() {
                      // Update the local data or refresh the Firestore data
                    });
                  } else {
  ...
  Future<dynamic> updateData(data) async {
    ...
    Map<String, dynamic> students = ({
      ...
      "dateOfEnquiry": DateFormat("dd-MMM-yyy").parse(
          enquirydateController.text)
    });
